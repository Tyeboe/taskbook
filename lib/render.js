...
class Render {
  ...
  displayStats({percent, complete, pending, notes}) {
    ...
    const status = [
      ...
      `${chalk[sig._types.note.color](notes)} ${grey('notes')}`
    ];
    let categories = {};
    categories.categoryKeyArray = [];
    categories.categoryInformationArray = {};
    notes.forEach(x => {
      let name = x.category[0]
      try{
        categories.categoryInformationArray[name][0]++;
      }
      catch(Exception){
        categories.categoryKeyArray.push(name);
        categories.categoryInformationArray[name] = [0,0]
        categories.categoryInformationArray[name][0] = 1;
        categories.categoryInformationArray[name][1] = x.category[1];
      }
    });
    categories.categoryKeyArray.forEach(cat => {
      let catCount = categories.categoryInformationArray[cat][0];
      let catColor = categories.categoryInformationArray[cat][1];
      status.push(`${chalk.hex(catColor)(catCount)} ${grey(cat)}`)
    });
