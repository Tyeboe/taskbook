...
class Taskbook {
  ...
  import(input) {
    if (input.length === 0) {
      render.noFileSpecified();
      process.exit(1);
    }
    if (input.length > 1) {
      render.toManyFilePathsSpecified(input.length);
      process.exit(1);
    }
    try {
      let raw_data = fs.readFileSync(input[0]);
      let taskbookData = JSON.parse(raw_data);

      this._save(taskbookData);
    } catch (error) {
      render.importFailed(input[0]);
      process.exit(1);
    }

    render.successImport(input[0]);
  }

  graph(input) {
    let boards = this._getBoards();
    let graphs = [];
    if (input.length === 0) {
      input = boards;
    }

    if (!input.every((board) => {
      if (!boards.includes(board)) {
        render.invalidBoardName(board);
        return false;
      }
      let stats = this._getStats(this._filterTask(Object.assign({}, this._data), (task) => task.boards.includes(board)));
      let graph = {};
      graph[board] = { stats };
      if (graph[board].stats.pending !== 0 || graph[board].stats.complete !== 0) {
        graphs.push(graph);
      }
  ...
  export(input) {
    ...
    const taskbookData = Object.assign({}, this._data);
    try {
      fs.writeFileSync(input[0], JSON.stringify(taskbookData, null, 4));
    } catch (error) {
      render.exportFailed(input[0]);
      process.exit(1);
    }

    render.successExport(input[0]);
  }
  ...
  _generateID(data = this._data) {
    ...
    const min = (ids.length === 0) ? 0 : Math.min(...ids);
      ...
      if (!this._isPriorityOpt(x)) {
        ...
        return x.startsWith('@') && x.length > 1 ? boards.push(x) :
          /^\d\d?\/\d\d?\/(\d\d)?\d\d$/.test(tempDate) && dueDate === null
            ? dueDate = new Date(tempDate) : desc.push(tempDate);
      }
    });
  ...
  displayItemsBefore(input) {
    let task_data = {};
    let beforePredicate = (item) => { return new Date(item._date) < new Date(input[0]); };
    Object.assign(task_data, this._data);
    this._filterTask(task_data, beforePredicate);
    let note_data = {};
    Object.assign(note_data, this._data);
    this._filterNote(note_data, beforePredicate);
    let data = {};
    Object.assign(data, note_data, task_data);
    render.displayByDate(this._groupByDate(data));
  }

  displayItemsAfter(input) {
    let task_data = {};
    let beforePredicate = (item) => { return new Date(item._date) > new Date(input[0]); };
    Object.assign(task_data, this._data);
    this._filterTask(task_data, beforePredicate);
  ...
  categorize(input) {
    const name = input.filter(x => x.startsWith('@'))[0] || "notes";
    const color = input.filter(x => x.startsWith('#'))[0] || "#0000ff";
    const modified = this.updateCategoryColors(name, color);
    const desc = [];
    input.forEach(x => {
      if (x != name && x != color) {
        desc.push(x);
    ...
    if (modified[0] && modified[1]) {
      render.successEditCat(name, color);
    }
    else if (modified[0]) {
      render.successCreateCat(name, color);
    }
  }
  ...
  deleteItems(ids) {
    let isRangePredicate = (id) => { return id.includes("-") && !id.includes("@") };
    let ranges = ids.filter(isRangePredicate);
    this._removeIf(ids, isRangePredicate);
    ranges.forEach((range) => {
      let rangeVals = range.split('-').filter((r) => r.length > 0);
      if (rangeVals.length != 2) {
        render.invalidRangeSpecification(range);
        process.exit(1);
      }
      for (let i = parseInt(rangeVals[0]); i <= parseInt(rangeVals[1]); i += 1) {
        ids.push(i.toString());
      }
    });
  ...
  updateCategoryColors(name, color) {
    ...
    Object.entries(_data).forEach(([id, item]) => {
      if (!item._isTask) {
        ...
        if (item.category[0] == name && item.category[1] != color) {
